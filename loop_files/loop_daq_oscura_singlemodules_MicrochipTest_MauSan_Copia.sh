...
#
#rutine in progress: 
rutine="Take Images with 324nsamp"
#
##
timeStart=$(date)
...
imgFOLDER=`dirname $BASH_SOURCE`/images/Microchip/ENE2024/EXPOSURE_0/NSAMP_324/400x700_A/ #END WITH / please!!!!
...
do
	...
	if [[ $newFitsImg == *.fits ]]
	then
		scp $newFitsImg detcluster@xook.nucleares.unam.mx:/storage/icn/detcluster/regular/.
	...
	#Image for serial read events
	lta sseq sequencers/sequencer_microchip_binned_brenda_VA.xml

	cols=700;rows=400; nsmpls=324; expo=0;
        doSettings

	lta name $imgFOLDER/skp_${runname}_Vv${vfile}_NSAMP_${nsmpls}_NROW_${rows}_NCOL_${cols}_EXPOSURE_${expo}_NBINROW_${binrow}_NBINCOL_${bincol}_SRE_img_
        lta read
	...
	#use skipper to root to process the last image
        ...
        if [[ $newFitsImg == *.fits ]]
        then
		scp $newFitsImg detcluster@xook.nucleares.unam.mx:/storage/icn/detcluster/sreImages/.
                mv $newFitsImg $imgFOLDER
        else
                echo "is not an image"
                echo $newFitsImg
