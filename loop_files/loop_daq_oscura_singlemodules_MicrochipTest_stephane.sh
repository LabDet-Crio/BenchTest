...
#
#rutine in progress: 
rutine="Take Images to test sequencers"
#
##
timeStart=$(date)
...
imgFOLDER=`dirname $BASH_SOURCE`/images/Microchip/stephanTest/ #END WITH / please!!!!
# path of ser test events files /home/detectores/Software/ltaDaemon-master/images/Microchip/serialRegTest/03NOV23/serTest
...
#Size  1058 x 1278 pix
CLEAR=120
cols=700; binrow=1; bincol=1;
vfile=82
for imagen in {1..1} #for para tomar imagenes con un mismo sequencer
#for var in HA HB VA VAHA VAHB VB VBHA VBHB HA HB VA VAHA VAHB VB VBHA VBHB #for para tomar imagenes con distintos sequencers
do
	if [ ! -f "$lockfilename" ]; then break; fi
		echo "Rutine $rutine"
	#source voltage_skp_lta_v${j}_microchip.sh
	#voltage_skp_lta_v2_microchip.sh
	source voltages/voltage_skp_lta_microchip_vTested_20230518.sh

	source eraseANDepurge.sh
	lta set vsub $Vvsub

	doClear
	#lta sseq sequencers/sequencer_microchip_binned_brenda_$var.xml #cambio de sequencers
	#lta sseq sequencers/sequencer_microchip_binned_brenda.xml  # clean Ser Reg Events 
        lta sseq sequencers/sequencer_microchip_binned_brenda.xml   # regular images

	cols=1400;rows=1300; nsmpls=1; expo=0;
	doSettings

	lta name $imgFOLDER/skp_${runname}_NSAMP_${nsmpls}_NROW_${rows}_NCOL_${cols}_EXPOSURE_${expo}_img_
	lta read
