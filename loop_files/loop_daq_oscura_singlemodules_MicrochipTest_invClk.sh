...
#
#rutine in progress: 
rutine="Take images 650x700 NSAMP 324 for muon and Serial Reg Events using Rev OS and regular pictures"
#
###
timeStart=$(date)
...
imgFOLDER=`dirname $BASH_SOURCE`/images/Microchip/serialRegTest/27OCT23/RevOS/ #END WITH / please!!!!
runname=m-009_microchip_vTested_T_170_
...
#for string in VA # VB HA HB VAHA VAHB VBHA VBHB
for loop in {1..10}
do
	...
	rows=650; nsmpls=225; expo=0;
	doSettings
	#lta name $imgFOLDER/skp_${runname}_seq_${string}_NSAMP_${nsmpls}_NROW_${rows}_NCOL_${cols}_EXPOSURE_${expo}_img_
